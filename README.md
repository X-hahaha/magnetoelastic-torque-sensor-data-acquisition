...
在 `ds_host_gui.py` 中点击“开始监测并记录”：接收 socket 会先绑定 PC UDP 50010，再发送带当前转速、L2阈值和采集点数的 `AUTO_START`，避免板端开始触发和建立记录线程之间丢失首帧。波形预览独立固定为最多 3000 个显示点，不改变实际采集点数。

按钮语义：

- **开始监测并记录**：先建立接收端，再使用当前转速、L2阈值和采集点数启动板端监测并记录；开始后三个参数输入框禁用。
