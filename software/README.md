...
界面保留转速、L2阈值和采集点数三个参数。转速范围为 `1~4000 rpm`，阈值为有符号 32 位微米值，采集点数范围为 `16~4,687,500`，默认值仍为 `3125`。波形预览显示点数独立固定为 3000，不会改变这里输入的实际采集点数。

- `开始监测并记录`：上位机先绑定 UDP 50010，再用当前转速、阈值和采集点数发送 `AUTO_START`，随后持续接收并落盘。按下后三个参数输入框都会禁用。
