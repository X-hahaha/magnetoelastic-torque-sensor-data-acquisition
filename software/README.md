...
- 顶部连接/命令栏：可修改板端 IP、本地绑定IP、端口和输出目录；“浏览”按钮右侧实时显示标定扭矩，并可点击“扭矩曲线”查看最近 60 秒的变化；同时支持 `Ping`、`DMA_STATUS`、`CAL_STATUS`、`CAL_START`、自定义 ASCII 命令和“采集一帧”。
...
实时扭矩使用 summary 中的 `me_2x_uV`，先对连续 15 帧做滑动平均（125 Hz 下为 120 ms），再按标定报告换算：

```text
T [N·m] = 0.4424 × (mean_15(me_2x_uV) − 1118.9) + 86.4
```

滤波窗口未满时，顶部读数会标为“预热 n/15”；窗口满后显示正式读数并写入扭矩曲线。开始一次新的手动采集或自动连续记录时，滤波窗口和曲线历史会清空。自动连续模式才能持续提供逐帧 summary，因此是实时扭矩显示的推荐使用方式。
